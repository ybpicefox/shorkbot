...
from config.constants.constants import Channel, Role
from utils.utils import staff_check, Embed, staff_only, get_file_json, save_file_json, MessageOrReplyConverter
...
class Tickets(commands.Cog):
    def __init__(self, bot):
        ...
        self.ticket_types = get_file_json("config/data/tickets")

    def load_message_ids(self):
        self.message_ids = get_file_json("config/data/config")["ticket_messages"]

    @commands.command()
    @staff_only
    async def addTicketMessage(self, ctx, message: discord.Message):
        config = get_file_json("config/data/config")
        config["ticket_messages"].append(message.id)
        save_file_json(config, "config/data/config")
        self.load_message_ids()
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
        if payload.message_id not in self.message_ids:
            return
        if str(payload.emoji) in self.ticket_types:
            guild: discord.Guild = self.bot.get_guild(payload.guild_id)
            msg: discord.Message = await guild.get_channel(payload.channel_id).fetch_message(payload.message_id)
            await msg.remove_reaction(payload.emoji, payload.member)
            embed = discord.Embed(title=self.ticket_types[str(payload.emoji)]["name"], colour=discord.Colour.green(),
            ...
            for question in self.ticket_types[str(payload.emoji)]["questions"]:
            ...
            await guild.get_channel(Channel.MOD_LOGS).send(
                embed=Embed(
                    ...
                    colour=0x00ff00,
                    title="Ticket opened",
                    description=f"{channel.name}\nReason: {self.ticket_types[str(payload.emoji)]['name']}"
                ).auto_author()
            )
